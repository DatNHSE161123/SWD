...
import { useCanceledModal, useDeliveredModal } from "@/hooks/useTransaction";
...
const TransactionManagement = () => {
    ...
    const { data: listTransaction, error, isLoading } = useSWR<TransactionDetail[]>(user ? `/transaction/getByUser?userId=${user._id}` : null, fetcher, { refreshInterval: 1000 })
                                    ...
                                    <div className="flex flex-row gap-2 justify-between mt-5">
                                        ...
                                        <div className="flex justify-end gap-2">
                                            {transaction.status === shipping && (
                                                <button className="bg-primary-cus text-white hover:bg-red-500 w-fit px-4 py-2 rounded-md text-lg font-semibold" onClick={() => deliveredModal.onOpen(transaction._id)}>
                                                    Xác nhận
                                                </button>
                                            )}
