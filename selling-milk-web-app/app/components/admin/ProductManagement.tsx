...
import { ManageProductData, TableProductProps } from "@/types";
import { useDeleteProductModal, useDiscountModal } from "@/hooks/useProduct";
...
const TableProduct: React.FC<TableProductProps> = ({ listProduct, currentPage, itemsPerPage }) => {
    ...
    const listAction = [
        { title: "Xem chi tiết sản phẩm", src: (productId: string) => router.push(`/admin/detail-product-management/${productId}`) },
        { title: "Xóa sản phẩm", src: (productId: string) => deleteProductModal.onOpen(productId) },
        { title: "Cập nhật giảm giá", src: (productId: string) => discountModal.onOpen(productId) },
                ...
                {listProduct.map((product, index) => {
                    ...
                    return (
                        <tr key={index}>
                            ...
                            <td className="py-3 border-r border-black border-opacity-10">{product.totalStock}</td>
                            <td className="py-3 border-r border-black border-opacity-10">{product.sale}</td>
                            <td className="py-3 border-r border-black border-opacity-10">{product.discount}%</td>
...
const ProductManagement = () => {
    ...
    const { data: listManageProduct, error, isLoading } = useSWR<ManageProductData[]>("/product", fetcher, { refreshInterval: 1000 })
        ...
        <>
            ...
            <div className="fixed bottom-5 right-5">
                <Button
                    ...
                    style="px-4 py-3"
                    onClick={() => router.push("/admin/post-product")}
                />
            </div>
        </>
    )
